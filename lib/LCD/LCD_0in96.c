...
void LCD_0IN96_Display_circular(UWORD *Image, short int offset)
{
    UWORD j,k;
    LCD_0IN96_SetWindows(0, 0, LCD_0IN96.WIDTH-1, LCD_0IN96.HEIGHT-1);
    DEV_Digital_Write(LCD_DC_PIN, 1);
	
    for (j = offset; j < LCD_0IN96.HEIGHT; j++) {
        DEV_SPI_Write_nByte((uint8_t *)&Image[j*LCD_0IN96.WIDTH], LCD_0IN96.WIDTH*2);	
    }
    for (k = 0; k < offset; k++) {
        DEV_SPI_Write_nByte((uint8_t *)&Image[k*LCD_0IN96.WIDTH], LCD_0IN96.WIDTH*2);	
    }
}
